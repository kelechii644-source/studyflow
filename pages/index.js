...
export default function StudyFlow() {
  ...
  const circumference = 2 * Math.PI * 90;return (
    <div style={{ minHeight: "100vh", background: "#0d0d14", fontFamily: "'Segoe UI', sans-serif", color: "#fff", maxWidth: "420px", margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ padding: "20px", paddingBottom: "90px" }}>{screen === "home" && (
          <div>
            ...
            {subjects.map(sub => (
              <div key={sub.id} style={{ marginBottom: "14px" }}>
                ...
                <div style={{ height: "8px", borderRadius: "4px", background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                  <div style={{ width: `${Math.min((sub.minutes / Math.max(...subjects.map(s => s.minutes))) * 100, 100)}%`, height: "100%", background: sub.color, borderRadius: "4px" }} />
                </div>
              </div>
            ))}
            {!isPremium && (
